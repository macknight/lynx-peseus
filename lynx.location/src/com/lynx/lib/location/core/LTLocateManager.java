...
import com.lynx.lib.location.util.LTAMapLocateResponse;
import com.lynx.lib.location.util.LTBMapDigester;
import com.lynx.lib.location.util.LTLocationUtil;
...
//	private static final String G_LM_URL = "http://www.google.com/glm/mmap";
	...
	private static LTLocationCenter loc_center;	
	private static final List<LTCoord> coords = new ArrayList<LTCoord>();
	...
	private static Handler loc_handler = new Handler() {
		...
		public void handleMessage(Message msg) {
			...
			if (loc_status == LOC_REQ_FIN || elapse_total >= cur_timeout) {
				...
				Log.d(Tag, "locate finished with status: " + loc_status + " and elapse: " + elapse_total);
				elapse_total = 0;
				loc_status = 0;
				LTLocateManager.loc_center.sendEmptyMessage(LTLocationCenter.LOC_REQ_FIN);
			}
		}
	};
	...
	private final LocationListener location_listener = new LocationListener() {
		...
		public void onLocationChanged(Location location) {
			double lat = LTLocationUtil.format(location.getLatitude(), 5);
			double lng = LTLocationUtil.format(location.getLongitude(), 5);
			LTCoord coord = new LTCoord(LTCoordSource.GPS, 
					lat, lng, (int)location.getAccuracy(), 
					System.currentTimeMillis() - location.getTime());
			if (location.getProvider().equals(LocationManager.NETWORK_PROVIDER)) {
				coord.setSource(LTCoordSource.NETWORK);
				coords.add(coord);
				loc_handler.sendEmptyMessage(NETWORK_LOC_REQ_FIN);
				return;
			}
			coords.add(coord);
			coord.setSource(LTCoordSource.GPS);
			loc_handler.sendEmptyMessage(GPS_LOC_REQ_FIN);
		}

		@Override
		public void onProviderDisabled(String provider) {
			Toast.makeText(loc_center.getContext(), "GPS服务未打开", Toast.LENGTH_SHORT).show();
		}

		@Override
		public void onProviderEnabled(String provider) {
			Toast.makeText(loc_center.getContext(), "GPS定位开启一次，将在60s后关闭", Toast.LENGTH_SHORT).show();
		}
	...
	public LTLocateManager(LTLocationCenter loc_center) {
		LTLocateManager.loc_center = loc_center;
		
		location_manager = (LocationManager)loc_center.getContext().getSystemService(Context.LOCATION_SERVICE);
	...
	public void cellLocate() {
		...
		// 存在基站时，发送辅助定位请求
		if (LTLocationCenter.getCells() != null && LTLocationCenter.getCells().size() > 0) {
			new GLocTask(LTLocationCenter.getCells(), null).locate();
		}
		else {
	...
	public void wifiLocate() {
		...
		// 存在基站时，发送辅助定位请求
		if (LTLocationCenter.getWifis() == null || LTLocationCenter.getWifis().size() < 2) {
		...
		aloc_task = new ALocTask(LTLocationCenter.getCells(), 
				LTLocationCenter.getWifis(), 
				LTLocationCenter.getNetworkInfo(),
				LTLocationCenter.getDhcpInfo());
		aloc_task.locate();
		
		bloc_task = new BLocTask(LTLocationCenter.getCells(), 
				LTLocationCenter.getWifis());
		bloc_task.locate();
	}
	...
	public void gpsLocate() {
		...
		if (location != null && SystemClock.elapsedRealtime() - location.getTime() < 30000) {
			long elapse = SystemClock.elapsedRealtime() - location.getTime();
			LTCoord coord = new LTCoord(LTCoordSource.GPS, 
					location.getLatitude(), location.getLongitude(), 
					(int)location.getAccuracy(), elapse);
			coords.add(coord);
			loc_handler.sendEmptyMessage(GPS_LOC_REQ_FIN);
		}
		else {
			location_manager.requestLocationUpdates(provider, 0, 0, location_listener);
		}
	}
	...
	private class GLocTask extends LocateTask {
		...
		private List<LTCell> cells = null;
//		private List<LTWifi> wifis = null;
		
		
		
		public GLocTask(List<LTCell> cells, List<LTWifi> wifis) {
			this.cells = cells;
//			this.wifis = wifis;
		}
	...
	private class ALocTask extends LocateTask {
		private List<LTCell> cells;
		private List<LTWifi> wifis;
		private NetworkInfo network_info;
		private DhcpInfo dhcp_info;

		public ALocTask(List<LTCell> cells, List<LTWifi> wifis, 
		...
		public void locate() {
			...
			if (this.wifis == null || this.wifis.size() == 0) {
				this.source = LTCoordSource.ACELL;
			}
			else {
				this.source = LTCoordSource.AWIFI;
			}
			...
			new Thread() {
				...
				public void run() {
					...
					try {
						...
						HttpConnectionParams.setConnectionTimeout(http_params, cur_timeout);
						HttpConnectionParams.setSoTimeout(http_params, cur_timeout);
						http_client = new DefaultHttpClient(http_params);
						http_post = new HttpPost(A_LOC_URL);
						StringEntity param = new StringEntity(str_param, "UTF-8");
						param.setContentType("text/xml");
						http_post.setHeader("Content-Type", "application/soap+xml;charset=UTF-8");
						...
						if ((http_resp.getHeaders("Content-Type") != null)) {
							String content_type = http_resp.getEntity().getContentType().getValue();
							String[] tmp = content_type.split("\\;");
							for (int i=0; i<tmp.length; ++i) {
								if (tmp[i].trim().toLowerCase().startsWith("charset=")) {
									charset = tmp[i].trim().toLowerCase().replace("charset=", "");
								}
							}
						}
						if ((http_resp.getHeaders("Content-Encoding") != null)) {
							Header[] headers = http_resp.getHeaders("Content-Encoding");
						...
						if (http_resp.getStatusLine().getStatusCode() == HttpStatus.SC_OK) {
							...
							if (http_entity != null) {
								...
								if (stream_type != null && "gzip".equals(stream_type)) {
									instream = new GZIPInputStream(instream);
								}
								BufferedReader buf_reader = new BufferedReader(new InputStreamReader(instream, charset));
								String line = null;
								...
								if (!"".equals(sb.toString())) {
									String result = LTAMapDeseder.decrpyt(new LTAMapLocateResponse().ParseSapsXML(sb.toString()), "GBK");
									LTCoord coord = new LTAMapLocateResponse().parseCoordFromXML(result);
									if (coord != null) {
										elapse = System.currentTimeMillis() - elapse;
										coord.setElapse(elapse);
										coord.setSource(LTCoordSource.AWIFI);
										Log.d(Tag, String.format("get coord(%s) form GaoDe(%f, %f, %d)", source,
						...
						loc_handler.sendEmptyMessage(A_LOC_REQ_FIN);
						Log.d(Tag, String.format("get coord(%s) from GaoDe done!", source));
					}
				}
			}.start();
		}
		...
		private String formatRequest() {
			...
			sb_param.append("<license>").append(AMAP_LICENSE).append("</license>");
			sb_param.append("<src>").append(PRODUCT_NAME).append("</src>");
			sb_param.append("<imei>").append(LTLocationCenter.getDeviceId()).append("</imei>");
			sb_param.append("<network>");
			if (network_info != null && network_info.isAvailable() && network_info.isConnected()) {
				TelephonyManager tel_manager = (TelephonyManager)loc_center.getContext().getSystemService(Context.TELEPHONY_SERVICE);
				sb_param.append(network_info.toString());
				sb_param.append(", countryiso: ").append(tel_manager.getNetworkCountryIso()).
				append(", operatorname: ").append(tel_manager.getNetworkOperatorName()).
				append(", line1number: ").append(LTLocationCenter.getPhone());
				
				if (dhcp_info != null) {
					sb_param.append(", wifidns1: " + LTLocationUtil.intToIpAddress(dhcp_info.dns1)).
						append(", wifidns2: " + LTLocationUtil.intToIpAddress(dhcp_info.dns2)).
						append(", wifigateway: " + LTLocationUtil.intToIpAddress(dhcp_info.gateway)).
						append(", wifiipaddr: " + LTLocationUtil.intToIpAddress(dhcp_info.ipAddress));
				}
			}
			sb_param.append("</network>");
			...
			if (cells != null && cells.size() > 0) {
				...
				if (cells.get(0).getType() == LTCellType.GSM) {
					sb_cells.append("<cdma>0</cdma>");
					sb_cells.append("<mcc>").append(cells.get(0).getMCC()).append("</mcc>");
					sb_cells.append("<mnc>").append(cells.get(0).getMNC()).append("</mnc>");
					sb_cells.append("<lac>").append(cells.get(0).getLAC()).append("</lac>");
					sb_cells.append("<cellid>").append(cells.get(0).getCID()).append("</cellid>");
					sb_cells.append("<signal>").append(cells.get(0).getASU()).append("</signal>");
/*				
					if (cells.size() > 1) {
						sb_cells.append("<nb>");
...
*/					
				...
				else if (cells.get(0).getType() == LTCellType.CDMA) {
					sb_cells.append("<cdma>1</cdma>");
					LTCell cell = cells.get(0);
					sb_cells.append("<mcc>").append(cell.getMCC()).append("</mcc>");
					sb_cells.append("<sid>").append(cell.getMNC()).append("</sid>");
					sb_cells.append("<nid>").append(cell.getCID()).append("</nid>");
					sb_cells.append("<bid>").append(cell.getLAC()).append("</bid>");
					sb_cells.append("<lon>").append("").append("</lon>");
					sb_cells.append("<lat>").append("").append("</lat>");
					sb_cells.append("<signal>").append(cell.getASU()).append("</signal>");
				}
			...
			if (this.wifis != null && this.wifis.size() > 0) {
				...
				for (LTWifi wifi : wifis) {
					sb_wifis.append(wifi.getMac()).append(",").append(wifi.getDBm()).append("*");
				}
			...
			sb_param.append("<clienttime>").append(LTLocationUtil.getCurrentTime()).append("</clienttime>");
			sb_param.append("</location>");
			...
			sb2.append("<sreq>").append(LTAMapDeseder.encrypt(sb_param.toString())).append("</sreq>");
			sb2.append("</saps>");
			return sb2.toString();
		}
	}
	...
	private class BLocTask extends LocateTask {
		...
		public BLocTask (List<LTCell> cells, List<LTWifi> wifis) {
			this.cells = cells;
			this.wifis = wifis;
		}
		...
		public void locate() {
			new Thread() {
				@Override
				public void run() {
					LTCoordSource source = LTCoordSource.UNKNOWN;
					if (wifis != null && wifis.size() > 0) {
						source = LTCoordSource.BWIFI;
					}
					else if (cells != null && cells.size() > 0){
						source = LTCoordSource.BCELL;
					}
					...
					try {
						...
						if(http_resp.getStatusLine().getStatusCode() == HttpStatus.SC_OK) {
							...
							if (status_code != 167) {						
								...
								lat = LTLocationUtil.format(lat, 5);
								double lng = json.getJSONObject("content").getJSONObject("point").getDouble("x");
								lng = LTLocationUtil.format(lng, 5);
								int acc = (int)json.getJSONObject("content").getDouble("radius");
								LTCoord coord = new LTCoord(source, lat, lng, acc, elapse);
								Log.d(Tag, String.format("get coord(%s) form Baidu(%f, %f, %d)", source,
										coord.getLat(), coord.getLng(), coord.getAcc()));
								coords.add(coord);
							}
						}
						...
						loc_handler.sendEmptyMessage(B_LOC_REQ_FIN);
						Log.d(Tag, String.format("get coord(%s) from Baidu done!", source));
					}
				}
			}.start();
		}
		...
		public String formatRequest() {
			...
			if (wifis != null && wifis.size() != 0) {
				...
				for (int i=0; i<wifis.size(); ++i) {
					tmp += wifis.get(i).getMac().replaceAll(":", "") + ";" + Math.abs(wifis.get(i).getDBm()) + ";|";
				}
				tmp = tmp.substring(0, tmp.length() - 1);
			}
			tmp += "&addr=detail&coor=gcj02&os=android&prod=default&im=";
			tmp += LTLocationCenter.getDeviceId();
			tmp = LTBMapDigester.digest(tmp) + "|tp=2";
	...
	private abstract class LocateTask {
		protected  LTCoordSource source = LTCoordSource.UNKNOWN;
