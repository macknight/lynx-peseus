...
public class LTAMapLocateResponse {
	...
	public LTCoord parseCoordFromXML(String input) {
		InputStream instream = new ByteArrayInputStream(input.getBytes());
		SAXParserFactory sax_factory = SAXParserFactory.newInstance();
	...
	private class LTContentHandler extends DefaultHandler {
		...
		public LTContentHandler(LTAMapLocateResponse resp) {
	...
	private class LTCoordHandler extends DefaultHandler {
		...
		public LTCoordHandler(LTAMapLocateResponse resp) {
			coord = new LTCoord();
			...
			if ("result".equals(localName)) {
				...
				if (coord != null) {
					try {
						...
						coord.setLng(LTLocationUtil.format(lng, 5));
					}
					catch (Exception e) {
						e.printStackTrace();
					}
				...
				if (coord != null) {
					try {
						...
						coord.setLng(LTLocationUtil.format(lat, 5));
					}
					catch (Exception e) {
						e.printStackTrace();
					}
