...
public class LTRGCManager {
	...
	private static LTAddress addr;
	private GRGCTask grgc_task;
	private static LTLocationCenter loc_center;
	
	public LTRGCManager(LTLocationCenter loc_center) {
		LTRGCManager.loc_center = loc_center;
	}
	
	public LTAddress getAddress() {
		return addr;
	}
	
	public void start() {
		grgc_task = new GRGCTask();
		grgc_task.setCoord(LTLocationCenter.getCoord());
		grgc_task.run();
	}
	...
	private static String getCityNameByCache(LTCoord coord) {
	...
	private static class GRGCTask implements Runnable {
		...
		public void run() {
			...
			if (coord == null) {
				loc_center.sendEmptyMessage(LTLocationCenter.LT_RGC_REQ_FIN);
				return;
			}
...
//									String short_name = ja_addr_components.getJSONObject(j).getString("short_name");
						...
						addr = new LTAddress(city, region, street);
						return;
					}
				}
			}
