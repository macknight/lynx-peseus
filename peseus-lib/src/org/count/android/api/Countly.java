...
public class Countly {
	...
	public void recordEvent(String key, Map<String, String> segmentation,
			int count) {
				...
				while (true) {
					...
					if (index != -1) {
						if (OpenUDIDManager.isInitialized() == false)
							break;
						data.replaceFirst("REPLACE_UDID",
								OpenUDIDManager.getOpenUDID());
					}

					try {
						DefaultHttpClient httpClient = new DefaultHttpClient();
						HttpGet method = new HttpGet(new URI(serverURL_ + "/i?"
								+ data));
...
class DeviceInfo {
	public static String getUDID() {
		return OpenUDIDManager.isInitialized() == false ? "REPLACE_UDID"
				: OpenUDIDManager.getOpenUDID();
	}
		...
		synchronized (this) {
			for (int i = 0; i < events_.size(); ++i) {
				...
				if (event.segmentation != null) {
					...
					String keys[] = event.segmentation.keySet().toArray(
							new String[0]);
		...
		synchronized (this) {
			for (int i = 0; i < events_.size(); ++i) {
				...
				if (event.key.equals(key)) {
					event.count += count;
					event.timestamp = (event.timestamp + (System
							.currentTimeMillis() / 1000.0)) / 2;
					return;
				}
			}
