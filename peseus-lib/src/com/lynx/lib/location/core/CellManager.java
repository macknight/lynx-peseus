 ...
 * 
 */
public class CellManager extends Handler {
	private LocationCenter loc_center;
	private TelephonyManager tel_manager;
	private final List<Cell> cells = new ArrayList<Cell>();
	private int asu = 0;

	public CellManager(LocationCenter loc_center) {
	...
	public void cellScan() {
		...
		if (cell_loc == null) {
			loc_center.sendEmptyMessage(LocationCenter.CELL_SCAN_FIN);
			return;
		}
		...
		case TelephonyManager.PHONE_TYPE_CDMA:
			...
			try {
				...
				cells.clear();
				Cell cell = new Cell(CellType.CDMA, mcc, sid, bid, nid, 0, lat,
						lng);
				cells.add(cell);
				loc_center.sendEmptyMessage(LocationCenter.CELL_SCAN_FIN);
			} catch (Exception e) {
				...
				try {
					...
					if (!(cid <= 0 || cid == 0xFFFF)) {
						...
						if (mcc == 0 && mnc == 0) {
							cells.clear();
							loc_center
									.sendEmptyMessage(LocationCenter.CELL_SCAN_FIN);
							return;
						}

						Cell cell = new Cell(CellType.GSM, mcc, mnc, cid, lac,
								asu, 0, 0);
						cells.add(cell);
					}
				} catch (Exception e) {
			...
			try {
				...
				if (neighbor_cells != null) {
					...
					for (NeighboringCellInfo n_cell : neighbor_cells) {
						...
						if (!(cid <= 0) || cid == 0xFFFF) {
							Cell cell = new Cell(CellType.GSM, mcc, mnc, cid,
									lac, n_cell.getRssi(), 0, 0);
							cells.add(cell);
						}
					}
					loc_center.sendEmptyMessage(LocationCenter.CELL_SCAN_FIN);
