...
public class PieChartView extends View {
	...
	public interface OnSliceClickListener {
		void onSliceClicked(PieChartView pieChart, int sliceNumber);
	}
	...
	protected void onDraw(final Canvas canvas) {
		...
		for (int i = 0; i < curSlice; i++) {
			...
			// Log.e("onDraw",
			// String.format("drawing previous slice %d: start: %f, size: %f",
			// i, startAngle, sliceSizes[i] ));
		...
		// Log.e("onDraw",
		// String.format("drawing current slice %d: start: %f, size: %f",
		// curSlice, startAngle, mCurrAngle - startAngle ));
		try {
			canvas.drawArc(bounds, startAngle, mCurrAngle - startAngle, true,
					paints[curSlice]);
	...
	public void startAnimation() {
		...
		shouldDraw = true;
		ValueAnimator animator = ValueAnimator.ofObject(new AngleEvaluator(),
				0.0f, 360.0f);
		animator.setDuration(1500); // TODO make method to provide duration
	...
	public boolean isOnPieChart(float x, float y) {
		...
		double distance = Math.sqrt(Math.pow(x - mCenterX, 2)
				+ Math.pow(y - mCenterY, 2));
	...
	public void setOnSliceClickListener(
			OnSliceClickListener onSliceClickListener) {
	...
	public void setOnClickListener(OnClickListener l) {
		throw new UnsupportedOperationException(
				"You should use setOnSliceClickListener() to set a click listener on a slice of the chart.");
	}
	...
	public boolean onTouchEvent(MotionEvent event) {
		if (onSliceClickListener != null) {
			if (event.getAction() == MotionEvent.ACTION_DOWN) {
				...
				Log.e("onTouchEvent",
						String.format("touch up - x: %f, y: %f", x, y));
				...
				if (inChart) {
					...
					for (int i = 0; i < sliceEndAngles.length; i++) {
						...
						Log.e("onTouchEvent",
								String.format(
										"slice %d, angle: %f, startAngle: %f, endAngle: %f",
										i, angle, startAngle, endAngle));
						if (angle >= startAngle
								&& angle <= (startAngle + sliceSize)) {
							slice = i;
							break;
						}
